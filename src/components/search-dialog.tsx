...
const SearchDialog = forwardRef<SearchDialogHandle, SearchDialogProps>(
  ({ searchData }, ref) => {
    ...
    const commentsDocs = useMemo(
      () =>
        searchData.filter((doc) =>
          doc._raw.flattenedPath.startsWith('comments/')
        ),
      [searchData]
    );
    ...
    const filteredDocs = useMemo(() => {
      ...
      return commentsDocs.filter((doc) => {
          ...
          <div className="mt-2 max-h-[300px] overflow-y-auto">
            {filteredDocs.length > 0 ? (
              <ul className="list-none p-0">
                {filteredDocs.map((doc) => (
                  ...
                  >
                    <Link
                      href={
                        doc.url?.startsWith('/comments/')
                          ? doc.url
                          : `/comments/${doc._raw.flattenedPath.replace(
                              /^comments\//,
                              ''
                            )}`
                      }
              ...
              <p className="text-sm text-center py-6">
                {query.length > 0 ? 'Nada encontrado.' : 'Digite para pesquisar'}
              </p>
            )}
          </div>
        </DialogContent>
